          ...
          <div className="footer-container___social-media-buttons">
            {
              socialMediaLinksData?.map((element) => {
                return <span className="footer-container__cards-icon" key={element.id}>
                  <a href={element.link} target="_blank" rel="noreferrer">{element.icon}</a>
                </span>
              })
            }
          </div>

          <p className="footer-container__rights-reserved"><FontAwesomeIcon icon={faCopyright} /> {" "}Shrey Thakkar. All rights reserved.</p>
        </div>
      </Box>
    </footer>
  );
